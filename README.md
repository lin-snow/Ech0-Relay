...
- **公开频道同步**：抓取 `t.me/s/<频道>` 的公开网页，无需任何 Telegram 凭证；把文本转成 markdown，图片以链接嵌入，并用原帖时间回填 `created_at`（时间线不乱）。
...
- **作用域**：`echo:read` + `echo:write`（读用于保留清理的计数/查询，写用于发帖和删除）
...
- **图片**：以 markdown `![](链接)` 形式嵌入 Telegram CDN 图链，不下载。链接理论上可能失效（后续可扩展为下载并走 Ech0 file 上传永久留存）。
